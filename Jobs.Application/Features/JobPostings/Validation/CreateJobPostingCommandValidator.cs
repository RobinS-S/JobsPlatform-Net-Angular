...
{
    public class CreateJobPostingCommandValidator : AbstractValidator<CreateJobPostingRequest>
    {
        public CreateJobPostingCommandValidator(IRepositoryBase<Company> repository)
        {
            RuleFor(x => x.Title)
            ...
            RuleFor(x => x.CompanyId)
                .MustAsync(async (id, _) => await repository.AnyAsync(new CompanyByIdSpecification(id), CancellationToken.None))
                .WithMessage("The given company does not exist.");
        }
    }
}
